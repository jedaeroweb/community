class AdsController < ApplicationController
  ...
  # GET /orders.json
  def index
    params[:per_page] = 18 unless params[:per_page].present?
    ...
    @ads = Ad.distinct('ads.id').joins(:speciality).joins(:address).where(condition).page(params[:page]).per(params[:per_page]).order('orders_count asc, id desc')

    @order_count = Order.joins(:speciality).where(order_condition).count
    @orders = Order.joins(:speciality).where(order_condition).page(0).per(12).order('id desc')

    @company_count = Company.distinct('id').joins(:company_specialities).joins(:specialities).where(company_condition).where.not( company_pictures_count: 0).count
    @companies = Company.distinct('id').joins(:company_specialities).joins(:specialities).where(company_condition).where.not( company_pictures_count: 0).page(0).per(12).order('id desc')

    @product_count = Product.joins(:speciality_category).where(product_condition).count
    @products = Product.joins(:speciality_category).where(product_condition).page(0).per(12).order('id desc')

    @blog_count = Blog.joins(:speciality_category).where(product_condition).count
    @blogs = Blog.joins(:speciality_category).where(product_condition).page(0).per(12).order('id desc')
  ...
  def ad_params
    params.require(:ad).permit(:ad_category_id, :address_id, :speciality_id, :title, :content, :start_date, :end_date, :budget, :budget_propose, ad_size_attributes: [:size_id], ad_floor_attributes: [:floor_id], ad_pictures_attributes: [:picture], ad_blueprints_attributes: [:picture]).merge(user_id: current_user.id)
  end
end
