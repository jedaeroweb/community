class Admin::ReservationsController < Admin::AdminController
  ...
  def update
    respond_to do |format|
      if @reservation.update(reservation_params)
        format.html { redirect_to [:admin, @reservation], notice: t(:message_success_update) }
  ...
  def reservation_params
    params.require(:reservation).permit(:title,:rating, :enable, review_content_attributes: [:content])
  end
end
