class Admin::UserPointLogsController < Admin::AdminController
  ...
  def index
    ...
    @user_point_log_count = UserPointLog.count
    @user_point_logs = UserPointLog.page(params[:page]).per(params[:per_page]).order('id desc')
  ...
  def create
    ...
    respond_to do |format|
      if @user_point_log.save
        ...
        format.html { redirect_to [:admin, @user_point_log], notice: t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @user_point_log.update(user_point_log_params)
        format.html { redirect_to [:admin, @user_point_log], notice: t(:message_success_update) }
