class Admin::SpecialityCategoriesController < Admin::AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @speciality_category.save
        format.html { redirect_to [:admin, @speciality_category], notice: t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @speciality_category.update(speciality_category_params)
        format.html { redirect_to [:admin, @speciality_category], notice: t(:message_success_update) }
  ...
  def set_speciality_category
    @notice = SpecialityCategory.find(params[:id])
  end

  # Never trust parameters from the scary internet, only allow the white list through.
  def speciality_category_params
    params.require(:speciality_category).permit(:title, :enable, notice_content_attributes: [:content])
  end
end
