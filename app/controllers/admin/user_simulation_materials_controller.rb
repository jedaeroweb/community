class Admin::UserSimulationMaterialsController < Admin::AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @user_simulation_material.save
        format.html { redirect_to [:admin, @user_simulation_material], notice: t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @user_simulation_material.update(user_simulation_material_params)
        format.html { redirect_to [:admin, @user_simulation_material], notice: t(:message_success_update) }
  ...
  def user_simulation_material_params
    params.require(:user_simulation_material).permit(:title)
  end
end
