class Admin::ContactsController < Admin::AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @contact.save!
        format.html { redirect_to admin_products_path, notice: 'contact was successfully created.' }
  ...
  def destroy
    @contact.destroy
    respond_to do |format|
      format.html { redirect_to admin_products_url, notice: 'contact was successfully destroyed.' }
  ...
  def contact_params
    params.require(:contact).permit(:contact_category_id, :user_id, :title, :content, :enable)
  end
end
