class Admin::SimulationMaterialsController < Admin::AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @simulation_material.save
        format.html { redirect_to [:admin, @simulation_material], notice: t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @simulation_material.update(simulation_material_params)
        format.html { redirect_to [:admin, @simulation_material], notice: t(:message_success_update) }
  ...
  def simulation_material_params
    params.require(:simulation_material).permit(:simulation_id, :material_id, :price, :enable, simulation_material_pictures_attributes: [:picture], simulation_material_content_attributes: [:content])
  end
end
