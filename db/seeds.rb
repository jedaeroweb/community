...
User.create!(:id => 7, :email => 'company5@jedaeroweb.co.kr', :name => '저장강박증', :password => 'a12345', :password_confirmation => 'a12345',user_pictures_attributes: [{ picture: File.open(Rails.root.join("public", "images", "7.jpg")) }], 'url'=>'https://www.jingyu.pe.kr')
User.create!(:id => 8, :email => 'company6@jedaeroweb.co.kr', :name => '예쁜간호사', :password => 'a12345', :password_confirmation => 'a12345',user_pictures_attributes: [{ picture: File.open(Rails.root.join("public", "images", "8.jpg")) }], 'url'=>'https://www.seonmi.pe.kr')
...
Talk.create!( user_id: 1, title: 'N100프로세서 컴퓨터가 최고의 가성비', tag_list: '가성비, 추천, PC, 미니PC, N100', content: '
요즘도 N100프로세서 미니 PC들이 싸게 계속 나오네요, 최근에는 10만원대면 미니 PC에 16램에 윈도우 포함인것을 살 수가 있네요
미쳤어요~~, 이건 윈도우를 사니 미니 PC를 주는 거네요
저는 N100미니 피시 대신 N100노트북을 쓰는데요 정말 좋아요
N100프로세서는 유투브 4K영상도 완전 잘 재생된다는 사실, 게다가 전기도 최고 조금 사용하니 단점을 찾기가 힘드네요
게임 안 할거면 다들 N100프로세서 컴퓨터 사서 돈 아끼고 전기도 아끼세요
', created_at: 1.day.ago)

Talk.create!( user_id: 1, title: '세계 TOP 5 최강국이였던 우리나라(조선)', tag_list: '역사, 조선, 강대국', content: '
사람들이 지금이 우리나라가 최고 국력이고 옛날 조선은 비리비리 한 나라로 알고 있는데 이는 사실과 다릅니다.
현재 우리나라는 군사, 경제력 따지면 대충 10위권 또는 밖이지요

하지만 조선시대 임진왜란 전을 보면 인구가 거의 2천만으로 이때 인구 2천만 넘는 나라는 전 세계에 손에 꼽았습니다.
지금도 인구가 중요하지만 이 떄는 다 고만고만한 기술에 인구가 국력의 바로미터 지요
일본도 전국시대가 막 끝난 시점이라 인구가 엄청 줄어서 조선보다 적었을것으로 예상됩니다.

① 중국 (명나라)

    1억~1억 5,000만 명

    당시 세계에서 가장 인구가 많은 나라

② 인도 (무굴 제국)

    1억~1억 4,000만 명

    북인도를 장악한 무굴 제국이 급성장하며 인구 증가

③ 신성 로마 제국 (독일+오스트리아 등 포함)

    2,000만~2,500만 명

    유럽에서 가장 인구가 많은 정치체였지만, 여러 국가들의 연합 형태

④ 프랑스 왕국

    1,500만~2,000만 명

    16세기 말에 유럽에서 가장 인구가 많은 단일 국가였을 가능성 큼

⑤ 오스만 제국 (터키, 중동, 발칸반도 포함)

    2,500만~3,000만 명

    당시 유럽, 중동, 북아프리카를 지배하는 대제국


전 세계 따져 봐도 이 정도 였는데,  이 중 신성로마제국은 사실 하나의 나라라고 보기 힘들지요
또한 프랑스 보다도 중앙집권 체제가 더 잘 잡혀 있었고요

그러니 따지고 보면 조선보다 확실히 쎈나라는 명나라, 무굴제국, 오스만제국 밖에 없었다고 볼 수 있습니다.
그 밑에서 조선=일본=프랑스 비슷한 국력이였습니다.

다만 그 후 임진왜란, 호란, 기근 등으로 이 때의 인구는 조선말에나 되어서 회복 되었다고 합니다.
', created_at: 1.day.ago)


Talk.create!( user_id: 1, title: '우분투 쓰레기 되었네', tag_list: '우분투, 리눅스', content: '
...
Talk.create!( user_id: 1, title: '리엑트네이티브 Mac 없이 github action으로 빌드 성공~!', tag_list: 'git, github', content: '
AI 랑 같이 해서 뚝딱뚝딱 겨우 만들고는 있는데…..

그냥 혼자 했으면 시간을 얼마나 더 썼을지

그렇게 해서 만든 거 이제 올리려고 하는데 mac컴퓨터 팔아서 없어서
github action 으로 iOS빌드 하는데 인증서 막힐 때 마다 시간 하루를 쓰네요……… ㅜ.ㅜ

이전에 mac 에서 빌드 안 해봤거나 Azure에서 빌드 안 해봤으면 이것도 못했을듯?

웬만한 사람은 이전에 성질나서 그만둘 가능성 80~90%


  이거 왜 그냥 한번에 되는것 같은 거 없나?

드디어 이틀만에 빌드 성공

하여간 Mac 없이 빌드 한걸로 고생한거 위안 받았네요



그런데 이렇게 고생해서 해봤자, 계산해보니 인건비도 안 나오게 생겼네요

슬슬 전직 해야겠습니다.')


Talk.create!( user_id: 1, title: '하하하, 개발자 도구 제한??', content: '
어느 청첩장 제작 사이트를 보고 있다가 개발자 도구로 봐볼라고 F12를 누르니 뜨는 경고창

"본 서비스는 민감함 개인정보를 보호하기 위해 개발자 도구 사용을 엄격히 제한 합니다.
개발자 도구 호출 시, 귀하의 정보가 로그로 기록되면 반복 시 서비스 이용이 제한될 수 있습니다."

ㅋㅋㅋ, 장난해?? F12 막혔구나 하면서 브라우저에서 메뉴로 개발자 도구 들어가니 당연히 그냥됨

하..............
예전부터 소스보기, 마우스 오른쪽 버튼 클릭, 복사-붙여넣기 막는 아무 소용없는 짓들, 마찬가지로 다 브라우저 메뉴에서 하면 됨
아무리 개발자 아니라도 개발자도구만 사용할줄 알아도 브라우저 메뉴에서 누르는 것은 다 할 줄 알텐데?
막히는 사람들이 있을거로 생각하고 만든거?? 웃으라고 만든거?? 짜증나라고 만든거??
')


Talk.create!( user_id: 1, title: '제발 맥북프로 좀 사지마', content: '
참 우리나라에 부자들 많긴 많나봐
초보라고 하면서 맥북프로 수백만원 짜리 척척 사고
자기돈 자기가 쓰는거 뭐라고 하기 뭣 하지만
그렇다 하더라도 굳이 그렇게 돈 낭비 할 필요 까지는 없는데

제발 우선 맥북에어로 써보고 쓰란 말이야
에어면 충분한데 프로도 아닌 놈들이 뭔 프로야
지들이 뭐 대단한것 한다고 수백만원을 다른나라에 기업에 가져다 바치는지
프로는 니가 그걸로 그정도 살 수 있는 돈 벌면 그 때 사란 말이야

이놈의 허세문화가 정말 문제지
애플이 참 급나누기 장사로 잘 해처먹네

세계적으로 맥북프로 구입 비율과 한국의 비율 통계 같은거 없나 모르겠네??
')


Talk.create!( user_id: 1, title: '북한의 현실과 반대인 정식명칭, 코미디', content: '
사실상 "왕정"인 북한의 정식명칭은 "조선 민주주의 인민 공화국"이다
애네들 하는 짓과 정식명칭과의 거리를 생각하면 코미디가 떠로 없다
"민주주의","인민","공화국"  ㅋㅋㅎ
